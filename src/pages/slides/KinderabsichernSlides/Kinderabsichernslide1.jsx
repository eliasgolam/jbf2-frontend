...
import { ReactComponent as SlideSVG } from './Kinderabsichernslide1.svg';
import { motion } from 'framer-motion';
import { useNavigate } from 'react-router-dom';

const Kinderabsichernslide1 = () => {
  const [step, setStep] = useState(1);
  const [isZoomed, setIsZoomed] = useState(false);
  const navigate = useNavigate();

  const maxSteps = 4; // nur bis Gruppe4
      ...
      <main className="flex justify-center items-center px-4">
        <div className={`transition-all duration-500 ${isZoomed 
          ...
          <div className="absolute bottom-6 left-0 right-0 flex justify-center gap-4 z-10">
            ...
            <button
              onClick={() => navigate('/kinderabsichern/slide2')}
              className="px-6 py-2 bg-white/80 text-[#4B2E2B] rounded-full hover:bg-white shadow-md transition"
            >
              Überspringen
            </button>
...
export default Kinderabsichernslide1;
