...
import { ReactComponent as SlideSVG } from './JBslide3.svg';
import { motion } from 'framer-motion';
import { useNavigate } from 'react-router-dom';

const JBslide3 = () => {
  const [step, setStep] = useState(-1); // Start bei -1, damit erster Klick Gruppe1 zeigt
  const [isZoomed, setIsZoomed] = useState(false);
  const navigate = useNavigate();

  const groupIDs = ['Gruppe1', 'Gruppe2', 'Gruppe3', 'Gruppe4', 'Gruppe5'];
  ...
  const nextStep = useCallback(() => {
    if (step < maxSteps - 1) {
      ...
      navigate('/beratung-starten'); // Am Ende zur Startseite
    }
  }, [step, navigate]);
    ...
    <div className="flex flex-col min-h-screen justify-between bg-cover bg-center text-[#4B2E2B]"
      ...
      <main className="flex justify-center items-center px-4">
        <div className={`transition-all duration-500 ${isZoomed
          ...
          >
            Unsere Beratung basiert auf einem strukturierten, klaren und persönlichen Prozess – abgestimmt auf deine Lebenssituation und Ziele.
          </motion.div>
          ...
          <div className="absolute bottom-6 left-0 right-0 flex justify-center gap-4 z-10">
            ...
            >
              {step < maxSteps - 1 ? 'Weiter' : 'Zurück zur Startseite'}
            </button>
