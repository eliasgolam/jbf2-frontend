...
import tragbarkeitAnimation from '../assets/Tragbarkeitsrechner.json';

const Immobilien = () => {
  const navigate = useNavigate();
  const { bereich } = useParams(); // Dynamischer Bereich (vermoegen, gesundheit, immoblie, etc.)
  ...
  const tools = [
    ...
    },
    {
      ...
      description: 'Berechne die Tragbarkeit für eine Immobilie inkl. Nebenkosten & Hypothek',
      animation: tragbarkeitAnimation,
      path: `/tools/${bereich}/tragbarkeitsrechner`,  // Dynamischer Bereich
    },
  ];
  ...
  return (
    ...
    >
      ...
      <main className="flex-grow max-w-6xl mx-auto px-4 py-8 text-center flex flex-col items-center">
        <h2 className="text-3xl font-bold mb-2">Immobilien-Analyse</h2>
        <p className="text-[#7E6B64] mb-8">
          Budgetplanung und Tragbarkeit verständlich und übersichtlich berechnen
        </p>
