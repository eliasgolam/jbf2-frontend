...
import ivRentenAnimation from '../assets/IVRentenrechner.json';
import slideStartenAnimation from '../assets/SlideStarten.json';

const LebenstandardTools = () => {
  const navigate = useNavigate();
  const { bereich } = useParams(); // Dynamischer Bereich aus URL (z.B. 'vermoegen', 'gesundheit', etc.)
...
const profilbild = user.profilbild || "/default-profile.png";
  ...
  const tools = [
    ...
    {
      title: 'Sparrechner',
      description: 'Vermögensentwicklung berechnen',
      animation: sparAnimation,
      path: `/tools/${bereich}/sparrechner`,  // Dynamischer Bereich
    },
    {
      title: 'Zinsvergleich',
      description: 'Zinssätze im Vergleich',
      animation: zinsAnimation,
      path: `/tools/${bereich}/zinsvergleich`,  // Dynamischer Bereich
    },
    {
      title: 'Starten oder Warten',
      description: 'Ertragsvergleich mit/ohne Wartezeit',
      animation: startenAnimation,
      path: `/tools/${bereich}/starten-oder-warten`,  // Dynamischer Bereich
    },
    {
      title: 'IV-Rechner',
      description: 'Lücke im Falle einer Invalidität erkennen & berechnen',
      animation: ivRentenAnimation,
      path: `/tools/:bereich/iv-rechner`,  // Dynamischer Bereich
    }
  ...
  return (
    ...
    >
      ...
      <main className="flex-grow max-w-6xl mx-auto px-4 py-8 text-center flex flex-col items-center">
        <h2 className="text-3xl font-bold mb-2">Lebensstandard sichern</h2>
        <p className="text-[#7E6B64] mb-8">Berechne Lücken & plane deine finanzielle Absicherung</p>
        ...
        <div
          ...
          onClick={() => navigate('/lebenstandard/slides')}
// Dynamische Navigation für Präsentation
        >
          <Lottie
            loop
...
export default LebenstandardTools;
